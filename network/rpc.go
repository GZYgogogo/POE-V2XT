...
const (
	...
	MessageTypeRequest    MessageType = 0x7
	MessageTypeReply      MessageType = 0x8
	MessageTypePrePrepare MessageType = 0x9
	MessageTypeVote       MessageType = 0xa
)
...
func DefaultRPCDecodeFunc(rpc RPC) (*DecodedMessage, error) {
	...
	case MessageTypePrePrepare:
		prePrepareMessage := new(PrePrepareMessage)
		if err := gob.NewDecoder(bytes.NewReader(msg.Data)).Decode(prePrepareMessage); err != nil {
			return nil, err
		}
		return &DecodedMessage{
			From: rpc.From,
			Data: prePrepareMessage,
		}, nil
	case MessageTypeVote:
		voteMessage := new(VoteMessage)
		if err := gob.NewDecoder(bytes.NewReader(msg.Data)).Decode(voteMessage); err != nil {
			return nil, err
		}
		return &DecodedMessage{
			From: rpc.From,
